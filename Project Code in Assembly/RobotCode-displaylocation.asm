...
LoadPosition:
        LOAD Zero
        ADDI 1
        STORE VARY  ;sets robot's Y-tile position to 1
        IN XPOS		;gets robot's actual X position (forward = x-axis)
        STORE Temp	;sets remainder to xpos to start
        
GetY:   LOAD Temp
		ADDI -581	;subtract 2 feet
		STORE Temp  ;save the remainder
        JNEG GetX   ;if remainder is 0 or negative, done
        JZERO GetX
        LOAD VARY   ;if remainder is greater than 0...
        ADDI 1		;...increase tile count by one
        STORE VARY
        JUMP GetY	;repeat

GetX:   LOAD Zero
		ADDI 3
        STORE VARX  			;sets robot's X-tile position to 3
        IN YPOS     			;gets robot's actual Y-position
        STORE Temp				;sets remainder as ypos to start
        JPOS GetXRight
        JNEG GetXLeft
        JZERO PositionDone
...
GetXRight: 	LOAD Temp
        	...
        	JZERO PositionDone 
        	LOAD VARX   		;if remainder is greater than 0
        	ADDI 1				;increase tile count by one
        	STORE VARX
        	JUMP GetXRight		;repeat
...
GetXLeft:   LOAD Temp
        	...
        	ADDI -1				;increase tile count by one
        	STORE VARX
        	JUMP GetXLeft		;repeat

PositionDone:
		LOAD VARX
		OUT LCD  	;displays VARX for debugging      
        RETURN
;/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
...
; Subroutine to Move Forward 2 feet
MoveForward:
	IN		THETA
...
VARX:		DW	0	;Robot's x-tile position
VARY:		DW	0	;Robot's y-tile position
