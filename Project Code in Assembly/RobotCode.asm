ORG 		&H0000  ; Begin program at x000

	JUMP	Init	; Begin program
	RETI			; Sonar Disabled
	JUMP	Done	; Jumps here at t = 56 sec
	RETI			; UART Interrupt disabled
	RETI			; Motor Stall interrupt disabled
;***************************************************************
;* Initialization
;***************************************************************
Init:
...
Start:	
        ...
        ADD  endTime			; set the timer interrupt in this block
        OUT	 CTIMER
        SEI  &B0010			; Activate only the timer interrupt
...
MainLoop:		
		LOAD LoopCount
		ADDI -4				;loop runs once per row, stops after 4th row (set to 1 for debugging now)
		JZERO Done
		...
		CALL BranchLeft
Loop2:	LOAD LoopCount
		ADDI 1				;increment loop counter
		STORE LoopCount
		
		CALL MoveBackward
				
		LOAD RealY			;update position
		ADDI -1
		STORE RealY
		LOAD Zero
		ADDI  3
...
BranchRight:
		...
		CALL MoveForward
		CALL CheckAbove		;check for objects
		CALL Wait
		CALL CheckBelow
		CALL MoveBackward	;return to the center aisle
		CALL MoveBackward
		LOAD LFlag			;if the left side is also clear, 
		ADDI -1
		JZERO NumLefts 		;and hasn't already been checked twice,
Caity:		
		CALL TurnLeft		;go straight back. otherwise return to center
		RETURN
NumLefts:
		LOAD LeftCount
		ADDI -1
...
BranchLeft:
        ...
        ADDI -2				;checking twice per side is enough
        JZERO EarlyReturn
		CALL TurnRight		;advance to the left tile
		JUMP QuickLeft
QuickCheck:
		LOAD QuickFlag
		ADDI 1
		STORE QuickFlag
QuickLeft:
		CALL MoveBackward	;move to tile closest to wall
		CALL MoveBackward
		LOAD Zero
		ADDI 1
		STORE RealX
		...
		CALL Wait
		CALL CheckBelow
		CALL MoveForward	;return to the center aisle
		CALL MoveForward
		CALL TurnLeft
...
CheckAbove:
	;check for objects above the robot
	LOAD Zero	;reset object distance and wall detection
	STORE ObjDist
	STORE OBJX
	STORE OBJY
...
ThreeTileA:
	...
	STORE ObjDist
	JUMP ObjectPosition
	LOAD RealX		;if you see an object 3 tiles away,
	ADDI -1			;don't check that side again
	JZERO LeftDone
	LOAD RealX
	ADDI -5
	JZERO RightDone
...
CheckBelow:
	;check for objects below the robot
	LOAD Zero 	;reset object distance and wall detection
	STORE ObjDist
	STORE OBJX
	STORE OBJY
...
ThreeTileB:
	...
	STORE ObjDist
	JUMP ObjectPosition
	LOAD RealX		;if you can see 3 tiles away,
	ADDI -1			;don't check again
	JZERO LeftDone
	LOAD RealX
	ADDI -5
	JZERO RightDone
...
WallFlag:   DW   0  ;1 if a wall is detected on branch
QuickFlag:  DW   0  ;1 if clear on both sides 
