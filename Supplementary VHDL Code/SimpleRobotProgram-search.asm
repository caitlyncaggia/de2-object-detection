...
MainLoop:
        ...
        CALL CheckRight  ;If the right side is clear, move right
        JPOS MoveRight

        LOAD HasMoved ;Check if we just moved
        JPOS MainLoop

        CALL CheckForward ;If the front is clear, move forward
        JPOS MoveForward

        LOAD HasMoved ;Check if we just moved
        JPOS MainLoop

        CALL CheckLeft  ;If the left is clear, move left
        JPOS MoveLeft
...
LoadPosition:
        LOAD Zero
        STORE VARY  ;sets robot's Y-tile position to 0
        IN XPOS		;gets robot's actual X position (forward = x-axis)
        
GetY:   SUB 581		;subtract 2 feet
        STORE Temp	;save the remainder
        JNEG GetX   ;if remainder is 0 or negative, done
        JZERO GetX
        LOAD VARY   ;if remainder is greater than 0
        ADD 1		;increase tile count by one
        JUMP GetY	;repeat

GetX:   OUT SSEG1	;prints VARY to 7-seg display for debugging
		LOAD Zero
        STORE VARX  ;sets robot's X-tile position to 0
        IN YPOS     ;gets robot's actual Y-position
        JZERO GetXCenter
        JNEG  GetXLeft

GetXCenter: LOAD  Zero
			ADD	  3
			STORE VARX 
			               
GetXLeft:   ADD 581		;subtract 2 feet
        	STORE Temp	;save the remainder
        	JPOS PositionDone  ;if remainder is 0 or positive, done
        	JZERO PositionDone 
        	LOAD VARY   ;if remainder is greater than 0
        	ADD 1		;increase tile count by one
        	JUMP GetXRight
        	
GetXRight: 	SUB 581		;subtract 2 feet
        	STORE Temp	;save the remainder
        	JNEG PositionDone ;if remainder is 0 or negative, done
        	JZERO PositionDone 
        	LOAD VARY   ;if remainder is greater than 0
        	ADD 1		;increase tile count by one
        	JUMP GetXRight
...
PositionDone:
		OUT SSEG2  ;prints VARX to 7-seg display for debugging      
        RETURN
;/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
