...
;/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
; All of this code ("GetAngle" through "DeadZone") is purely for example.
; It attempts to gently keep the robot facing 0 degrees, showing how the
; odometer and motor controllers work can be used
GetAngle:
...
CheckAngle:
	...
	JPOS   TurnRight   ; if angle error positive, turn right
	
TurnLeft: ; if angle wasn't positive, it's 0 or negative
	; If the angle is small, we don't want to do anything.
	ADD    DeadZone
	JPOS   NoTurn
	; otherwise, turn CCW
	LOAD   RSlow       ; RSlow value is defined near the bottom of this file
	JUMP   SendToMotors
TurnRight:
	SUB    DeadZone    ; if near 0, don't turn
	JNEG   NoTurn
	LOAD   FSlow       ; FSlow is defined near the bottom of this file
	JUMP   SendToMotors
NoTurn:
